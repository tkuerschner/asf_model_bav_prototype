...
terr_data <- g4 %>%
  ...
  #    y <= max(gr2$y) + buffer_size
  #) %>%
  dplyr::select(x, y, quality,iteration)
...
grp_path_bp <- ggplot() +
  ...
  new_scale_fill() +
  geom_tile(data = terr_data, aes(x = x, y = y, fill = quality, group = seq_along(iteration)), alpha = .5) +
  ...
  new_scale_fill() +
  geom_point(data = ap_data, aes(x = x, y = y,group = seq_along(iteration)), size = 2, color = "#003cff", fill = "blue", shape = 22) +
  new_scale_color() +
  geom_point(data = disp_event, aes(disperser_group_x, disperser_group_y, color = as.factor(disperser_id)))+
  geom_path(data = disp_event, aes(disperser_group_x, disperser_group_y, color = as.factor(disperser_id)))+
