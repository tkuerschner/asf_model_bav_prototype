...
groups2 <- groups %>%
  ...
  disp_event <- dispersers
  gr1 <- groups2 %>% filter(group_id == 1) %>% group_by(year) #%>%
  #sample_n(1)
...
grp_path_bp <- ggplot() +
  ...
  new_scale_fill() +
  geom_tile(data = terr_data, aes(x = x, y = y, fill = as.factor(territory_of_group)), alpha = .5) +
  geom_point(data = gr2, aes(tx, ty, group = group_id), fill = "#2ab1d3", colour = "#0d5a6d", size = 2, shape = 22) +
  new_scale_fill() +
  geom_point(data = gr2, aes(x = cx, y = cy), size = 2, color = "#003cff", fill = "blue", shape = 22)+ 
  #geom_point(data = ap_data, aes(x = x, y = y), size = 2, color = "#003cff", fill = "blue", shape = 22) +
  new_scale_color() +
...
#plot the data
ggplot(cd3, aes(x = as.factor(month), y = meanAP)) +
  geom_point() +
  geom_errorbar(aes(ymin = meanAP - sdAP, ymax = meanAP + sdAP), width = 0.2) +
...
ggplot(cd4, aes(x = as.factor(month), y = meanAP, color = as.factor(year))) +
...
ggplot(cd6, aes(x = n_clusters, y = prob_cluster, color = as.factor(month))) +
  geom_line() +
  labs(title = "Monthly distribution of cluster sizes", x = "Cluster size", y = "Probability") +
  theme_minimal()
