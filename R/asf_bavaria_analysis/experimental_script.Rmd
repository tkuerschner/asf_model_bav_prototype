...
groups2 <- groups %>%
  mutate(year = as.numeric(round(iteration / 365)) + 1)
  disp_event <- dispersers
  gr2 <- groups2 %>% filter(group_id == 5) %>% group_by(iteration) %>%
  sample_n(1)
...
```{r single groups ap over time}

#random group

rg1 <- groups %>% filter(group_id == 5, age_class != "Piglet") %>% group_by(iteration) %>% sample_n(1)


# get coords of all the ap of group

ap_coords <- rg1 %>%
  mutate(ap_list = gsub("\\[|\\]", "", ap_list)) %>%
  mutate(ap_coords = strsplit(as.character(ap_list), ";")) %>%
  unnest(ap_coords) %>%
  separate(ap_coords, into = c("ap_x", "ap_y"), sep = "_", convert = TRUE) %>%
  dplyr::select(iteration, ap_x, ap_y)

head(ap_coords)
unique(ap_coords$ap_x)
#plot one map for each iteration

for (i in unique(ap_coords$iteration))
{
  ap_coords2 <- ap_coords %>% filter(iteration == i)
  
  #ap_coords2 <- ap_coords2 %>%
  #  mutate(x = ((ap_x * 50) + 50/2) + min(habitat_bavaria_df$x),
  #         y = ((ap_y * 50) + 50/2) + min(habitat_bavaria_df$y))
  #
  #ap_coords2_sp <- ap_coords2 %>%
  #  st_as_sf(coords = c("x", "y"), crs = 3035) %>%
  #  as_Spatial()
  
  #cp_ap <- mcp(ap_coords2_sp, unin = c("m"), unout = c(("km2")))
  
  #summary(cp_ap$area)

  buffer_size <- 20

  head(g4$territory_of_group)

  terr_data2 <- g4 %>%
    filter(territory_of_group == 5) %>%
    filter(
      x >= min(g4$x) - buffer_size &
        x <= max(g4$x) + buffer_size &
        y >= min(g4$y) - buffer_size &
        y <= max(g4$y) + buffer_size
    ) %>%
    dplyr::select(x, y, quality, iteration, territory_of_group)
  
  ggplot() +
    geom_tile(data = terr_data2, aes(x = x, y = y, fill = quality)) +
    geom_point(data = ap_coords2, aes(x = ap_x, y = ap_y), size = 8, color = "#cd1068", fill = "#e10d69", shape = 18) +
    #geom_polygon(data = cp_ap, aes(x = long, y = lat, group = group), fill = NA, color = "#1f1f20") +
    scale_fill_viridis_c() +
    theme_minimal() +
    theme(
      axis.text = element_blank(),
    ...
    labs(title = paste("Day:", i))
  
  ggsave(paste0("./R/asf_bavaria_analysis/visuals/ap_over_time/ap_", i, ".png"), width = 25, height = 25, units = "cm", dpi = 300)
}
