...
gr2 <- gr1 %>% ungroup() %>% dplyr::select(iteration,x,y)
...
grp_path_bp <- ggplot()+
       geom_tile(data = mpz_data, aes(x=x, y=y, fill =quality))+
       scale_fill_gradient(low = "#c2e9cf", high = "#046104")+
        geom_point(data = gr2, aes(x, y),size = 3, color = "#dd4367")+
        geom_point(data = ap_data ,aes(x=x, y=y), size = 4, color = "#003cff")+
         #labs(title = "iteration: {frame_time}")
         labs(title = "iteration: {next_step}")

#summary(mpz_data)
#summary(gr2)

animated_plot <- grp_path_bp +
  transition_states(iteration, transition_length = 0.2, state_length = 0.2) +
  enter_fade() +
  exit_fade()
