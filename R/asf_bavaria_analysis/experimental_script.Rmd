...
gr1 <- groups %>% group_by(iteration) %>% sample_n(1)
...
gr2 <- gr1 %>% ungroup() %>% dplyr::select(iteration, x, y, movement_type, tx, ty)
...
grp_path_bp <- ggplot()+
        ...
        geom_path(data = gr2, aes(x = x, y = y, color = movement_type),  linewidth = 1) +
        geom_point(data = gr2, aes(x, y, group = seq_along(iteration), color = movement_type),size = 3)+
...
#summary(mpz_data)
#summary(gr2)
anim_save("example_movement10.gif", animate(animated_plot))
