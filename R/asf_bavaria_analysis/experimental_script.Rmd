...
carcass_data <- carcasses %>%
  ...
  animated_plot_carcass <- ggplot() +
  geom_tile(data = g2, aes(x = x, y = y, fill = quality)) +
  scale_fill_gradient(low = "#c2e9cf", high = "#046104") +
  labs(fill = "Habitat quality") +
  new_scale_fill() +
  geom_tile(data = carcass_data, aes(x = carcass_x, y = carcass_y, fill = n_carcasses)) +
  scale_fill_gradient(low = "white", high = "red") +
  theme_minimal() +
  theme(
    axis.text = element_blank(),
  ...
  animated_plot_carcass <- animated_plot_carcass + transition_states(iteration, transition_length = 0.5, state_length = 0.5) +
  labs(title = "Day: {frame_along}")

  anim_save("./R/asf_bavaria_analysis/visuals/carcass_animation.gif", animate(animated_plot_carcass))
